...
def load_and_process_data(folder_path: Path, intervention_type: int, intervention_mode: int, world_size=10):
...
# ---------- 主绘图 ----------
def create_plots(root_path: Path):
    ...
    world_dirs = [d for d in root_path.glob("world*") if d.is_dir()]
    if not world_dirs:
    ...
    for plot_type in ['reward_diff', 'q_diff_diff']:
        ...
        for wdir in world_dirs:
            # 6行4列布局：行是干预类型，列是干预模式
            fig, axes = plt.subplots(6, 4, figsize=(16, 18), sharex=True, sharey=True)
            
            for type_idx, int_type in enumerate([0, 1, 2, 3, 4, 5]):  # 6种干预类型
                for mode_idx, mode in enumerate([1, 2, 3, 4]):  # 4种干预模式
            ...
            plt.tight_layout()
            out = root_path / f"expanded_{plot_type}_{wdir.name}.png"
