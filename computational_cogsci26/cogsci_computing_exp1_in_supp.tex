...
In an environment with two pellet-generating regions, we measure \textit{fair coordination} through two metrics: (1) \textbf{Policy Divergence}: The degree to which agents specialize in different regions, measured by the difference in their ``first-region preference'' (Expected Reward in Region 1 minus Region 2). Larger values indicate greater spatial differentiation. (2) \textbf{System Equilibrium}: The balance of resource acquisition, measured by the absolute score difference between agents. Smaller values indicate fairer resource distribution.                                                                                                                                                                                                                                  
 A factorial ANOVA on Policy Divergence revealed significant main effects of teaching style ($F(3, 4704) = 418.96$, $p < .001$), interpretation type ($F(5,4704) = 9.03$, $p < .001$), and intervention rate ($F(3, 4704) = 8.88$, $p < .001$), as well as significant teaching style $\times$ interpretation type ($F(15, 4704) = 2.86$, $p < .001$) and teaching style $\times$ rate ($F(9, 4704) = 6.26$, $p < .001$) interactions. For System Equilibrium, teaching style ($F(3, 4704) = 267.02$, $p < .001$) and intervention rate ($F(3, 4704) = 82.09$, $p < .001$) were significant, while interpretation type alone was not ($F(5, 4704) = 1.56$, $p = .17$); however, its interaction with teaching style was significant ($F(15, 4704) = 2.99$, $p < .001$), indicating that interpretation effects depend on the teaching context.                                                                                                                                                                                                                                                           
 Figures~\ref{fig:6.2.21} and~\ref{fig:6.2.22} present these results. While all teaching styles improved Policy Divergence relative to no intervention, their effects on System Equilibrium diverged sharply. The \textit{Restart} style yielded significantly worse equilibrium than all other styles ($t = 21.98$, $p < .001$, $d = 0.87$; $M_{\text{Restart}} = 2626$ vs.\ $M_{\text{Others}} = 1167$), indicating that abrupt relocations to non-productive areas destabilize fair resource sharing. The \textit{Undoing} style emerged as the most \textit{balanced} intervention: it produced moderate Policy Divergence ($M = 74.0$) while maintaining strong System Equilibrium ($M = 949.8$). Although \textit{Restart} generated the highest raw Policy Divergence ($M = 165.7$), this came at the cost of severe equilibrium degradation. Thus, \textit{Undoing}---by returning agents to their previous positions---effectively penalizes encroachment on suboptimal paths without introducing the destabilizing effects of more aggressive relocations. The \textit{Correcting} style, which redirects agents toward nearby spawn tiles, required high intervention rates ($\geq 0.75$) to achieve both objectives. At high rates, Correcting produced significantly better System Equilibrium than at low rates ($t = 12.26$, $p < .001$, $d = 0.71$; $M_{\text{high}} = 478$ vs.\ $M_{\text{low}} = 1421$), suggesting that consistent corrective feedback is necessary to translate spatial guidance into balanced outcomes.             
     
 Turning to \emph{interpretation types}, the \textit{Reset} rule---which treats interventions as neutral relocations without updating action values---performed significantly worse than other interpretations across teaching styles ($t = -5.53$, $p < .001$, $d = -0.19$; $M_{\text{Reset}} = 86.3$ vs.  $M_{\text{Others}} = 103.3$). More strikingly, the \textit{Disrupt} interpretation showed a pronounced mismatch with the \textit{Correcting} style: when agents interpreted corrective relocations as punitive signals, Policy Divergence dropped sharply ($t = -13.12$, $p < .001$, $d = -0.77$;                
 $M_{\text{Disrupt+Correcting}} = 58.9$ vs.\ $M_{\text{Disrupt+Others}} = 125.9$). This pedagogical mismatch---where well-intentioned guidance is misread as punishment---actively undermines the differentiation of labor.                                                                                            
 These findings highlight a key \textbf{pedagogical matching principle}: successful coordination requires not only appropriate teaching actions but also compatible learner interpretations. When the agent's heuristic aligns with the teacher's intent (e.g., treating corrections as suggestions rather than disruptions), interventions accelerate spatial specialization; when they conflict, even beneficial guidance can impede coordination.        
...
\begin{figure}[htbp]
    ...
    \label{fig:6.2.1}
\end{figure}
 In an environment with three regions and three agents, we measured \textit{spatial attachment}---the degree to which agents concentrate their visits within their nearest region---as an index of emergent niche specialization. A factorial ANOVA revealed that all experimental factors significantly influenced Region Attachment: teaching style ($F(3, 4704) = 8517.27$, $p < .001$), interpretation type ($F(5, 4704) = 55.92$, $p < .001$), and intervention rate ($F(3,4704) = 455.05$, $p < .001$). All two-way and three-way interactions were also significant ($p < .001$), indicating that the effectiveness of any pedagogical strategy depends jointly on how interventions are delivered and how agents interpret them.                                  
 Figure~\ref{fig:6.2.1} illustrates these patterns. The \textit{Undoing} and \textit{Correcting} styles produced dramatically higher spatial attachment than the other styles ($t = 100.37$, $p < .001$, $d = 2.90$; $M_{\text{Undoing+Correcting}} = 586.6$ vs.\ $M_{\text{Others}} = 208.2$). Examining styles individually, \textit{Correcting} yielded the highest attachment ($M = 607.9$), followed closely by \textit{Undoing} ($M = 565.2$), while \textit{Exploration-encouraging Correcting} ($M = 299.3$) and \textit{Restart} ($M = 117.1$) lagged considerably behind. These results confirm that interventions which guide agents toward productive locations---or constrain them to previously visited states---function as a \textit{positive feedback loop} for spatial coordination, implicitly solidifying each agent's role within an emerging division of labor.                                                        
 Intervention rate amplified these effects, but not uniformly across styles. Correlation analyses revealed significant positive relationships between rate and Region Attachment for \textit{Undoing} ($r = .65$, $p < .001$), \textit{Correcting} ($r = .48$, $p < .001$), and \textit{Exploration-encouraging Correcting} ($r = .38$, $p < .001$). Critically, \textit{Restart} exhibited a \textit{negative} correlation ($r = -.24$, $p < .001$): higher intervention rates further \textit{reduced} spatial attachment, actively dismantling emerging niches rather than reinforcing them. This divergent pattern underscores that more intervention is not universally beneficial---its effect depends entirely on the pedagogical content of the relocation.                             
 The \textit{Exploration-encouraging Correcting} style produced a more diffused spatial footprint ($M = 299.3$); while agents remained primarily in their assigned regions, they exhibited higher cross-regional foraging than under \textit{Undoing} or \textit{Correcting}. This confirms that this style prioritizes individual policy robustness---exposing agents to a broader state space---over strict group coordination.                                        
 The \textit{Restart} style proved unambiguously detrimental to spatial coordination ($t = -108.51$, $p < .001$, $d = -2.29$; $M_{\text{Restart}} = 117.1$  vs.\ $M_{\text{Others}} = 490.8$). By relocating agents to non-productive tiles far from spawn regions, \textit{Restart} erased the spatial structure that other styles cultivated, leading to chaotic, uncoordinated foraging. The massive effect size ($d = 2.29$) highlights that this is not a marginal difference but a qualitative failure of the pedagogical strategy. 
...
However, its effectiveness as a stabilizer is contingent upon the agent's interpretation, as evidenced by its severe incompatibility with the \emph{Disrupt} rule. This observation refines the pedagogical matching principle: a well-intentioned guiding correction can fail if the learner misinterprets it as a cautionary demonstration.
